...
def parse_args():
    ...
    # environment variant selection (only used when correlated_with_colour is False)
    parser.add_argument('--env_variant', default=None, choices=['env1', 'env2'], help='Select a single environment variation: env1 uses colourA/xml A, env2 uses colourB/xml B. Ignored if correlated_with_colour is True.')
    ...
    # Enforce variant only when correlation is disabled
    if not args.correlated_with_colour:
        assert args.env_variant in ('env1', 'env2'), 'must provide --env_variant env1|env2 when --correlated_with_colour is False'
    else:
        # Ignore env_variant if correlation is enabled
        args.env_variant = None
