...
# Ensure a valid MuJoCo OpenGL backend is selected before importing dm_control/dmc2gym
if 'MUJOCO_GL' not in os.environ:
    # Prefer EGL on headless Linux; leave as-is on desktop setups
...
class Workspace(object):
    def __init__(self, cfg):
        ...
        # - If correlated_with_colour is True, keep using provided exp_name (backward compatible)
        # - If False, save under {domain_name}_{env_variant}_{algorithm}/{algorithm}/{seed}
        if cfg.correlated_with_colour:
            exp_dir_name = cfg.exp_name
        else:
            env_variant = getattr(cfg, 'env_variant', None) or 'env'
            exp_dir_name = f"{cfg.domain_name}_{env_variant}_{cfg.algorithm}"
        self.work_dir = os.path.join(os.getcwd(), cfg.log_dir, exp_dir_name, cfg.algorithm, str(cfg.seed))
        ...
        #     else:
        #         self.envs = False
        #         # When not using correlation, force a specific env variant
        #         # env1 -> xml A + colourA, env2 -> xml B + colourB
        #         self.static_env_variant = None
        #         self.static_colour = None
        #         if getattr(self.cfg, 'env_variant', None) in ('env1', 'env2'):
        #             original_rgb = np.copy(self.env.physics.model.mat_rgba)[:, :3]
        #             self.colourA = np.copy(original_rgb)
        #             self.colourB = np.copy(original_rgb)
        #             self.colourA[1, :] = [0., 0., 1.0]
        #             self.colourB[1, :] = [0., 1.0, 0.]
        #             if self.cfg.env_variant == 'env1':
        #                 xml_path = os.path.join("world_models", f"{cfg.domain_name}_A.xml")
        #                 self.static_colour = self.colourA
        #             else:  # env2
        #                 xml_path = os.path.join("world_models", f"{cfg.domain_name}_B.xml")
        #                 self.static_colour = self.colourB
        #             # Reload the physics to the chosen variant and set the colour once
        #             self.env.physics.reload_from_xml_path(xml_path)
        #             self.static_env_variant = self.cfg.env_variant
        #             self.env.reset(colour=self.static_colour)
        #         else:
        #             self.env.reset()

        if cfg.domain_name == 'carracing':
            if cfg.env_variant == 'env1':
                from envs.carracing.car_racing import CarRacing
                env = CarRacing(
                    continuous=True,
                    background="green")  # , image_path=args.image_path)

            if cfg.env_variant == 'env2':
                from envs.carracing.car_racing_slow import CarRacing
                env = CarRacing(
                    continuous=True,
                    background="red")  # , image_path=args.image_path)

            env = utils.make_env_atari_single(env,
                seed=cfg.seed,
                rgb=True,
                stack=4,
                no_op=0,
            ...
            # env = utils.GymV21StepAPIWrapper(env)

            # Register as active env
            self.train_env = env
            self.env = env
            self.envs = False
    ...
    def evaluate(self):
        ...
        for episode in range(self.cfg.num_eval_episodes):
            ...
            if self.cfg.correlated_with_colour:
                ...
                # If a static variant was selected, keep resetting with its colour
                if getattr(self, 'static_env_variant', None) in ('env1', 'env2'):
                    obs = eval_env.reset(colour=self.static_colour)
                else:
                    obs = eval_env.reset()
        ...
        while self.step <= (total_num_steps + 1):
            if done:
                ...
                if self.cfg.correlated_with_colour:
                    ...
                    if getattr(self, 'static_env_variant', None) in ('env1', 'env2'):
                        obs = self.env.reset(colour=self.static_colour)
                    else:
                        obs = self.env.reset()
                prev_obs = obs.copy()
            ...
            # Treat time-limit truncations as non-terminal for bootstrapping.
            # Gymnasium hides _max_episode_steps; rely on info flags propagated by wrapper.
            truncated = info.get('truncated') or info.get('TimeLimit.truncated')
